...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  near=cam_info.near, far=cam_info.far, timestamp=cam_info.timestamp, rayo=rays_o, rayd=rays_d)
...
def loadCamv2(args, id, cam_info, resolution_scale):
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, 
                  image=gt_image, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, uid=id, data_device=args.data_device, #있던거
                  near=cam_info.near, far=cam_info.far, 
...
def cameraList_from_camInfosv2(cam_infos, resolution_scale, args, ss=False):#이거 씀
    camera_list = []
    if not ss: #이거 씀
        for id, c in enumerate(cam_infos):
